...
export type LoadLegacySaveDocumentResult =
  | {
      readonly status: 'loaded';
      readonly saveVersion: typeof LEGACY_SAVE_VERSION;
      readonly sourceSaveVersion: 1 | typeof LEGACY_SAVE_VERSION;
  ...
  | {
      ...
      readonly targetSaveVersion: typeof LEGACY_SAVE_VERSION;
      readonly issues: readonly SaveV2ValidationIssue[];
    };
...
/** Frozen v1/v2 interpretation for read-only migration and historical tests. */
export function loadLegacySaveDocument(input: unknown): LoadLegacySaveDocumentResult {
  ...
  if (saveVersion === 1) {
    ...
    return loadedV2.status === 'invalid'
      ? {
          ...
          sourceSaveVersion: 1,
          targetSaveVersion: LEGACY_SAVE_VERSION,
  ...
  return loaded.status === 'loaded'
    ? {
        status: 'loaded',
        saveVersion: LEGACY_SAVE_VERSION,
        sourceSaveVersion: LEGACY_SAVE_VERSION,
