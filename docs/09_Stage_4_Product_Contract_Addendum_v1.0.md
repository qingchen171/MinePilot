...
Historical recovery note：本节当时的 S4-08.2 Design Review entry 已由下方 §15 的 PASS/CLOSED 状态取代；当前唯一入口以 §15 和 PROJECT_STATUS 为准。

## 15. S4-08.2 Runtime / Mapping Foundation Design — PASS / CLOSED (2026-09-20)

**FINAL DESIGN CONTRACT APPROVED, INCLUDING LEGACY TRUST AND ACCOUNT ISOLATION CORRECTIONS.** 本节不实施代码；production仍为Stage3 Runtime/Save v2，`CURRENT_SAVE_VERSION=2`、v3 writer disabled。

- Target/dormancy：最终 root 为 `Stage4GameState { account, currentAttempt: AttemptState | null }`，Account always exists、最多一个 Attempt、null为真实account-only；Attempt拥有runId/levelId/nullable historical provenance/Run/RunItems/Rewards/temporary card/terminal disposition，revision与storage coordination metadata不进入Runtime。S4-08.2 foundation只供direct tests、mapper/reconstruction、dormant reader与S4-08.3 tests，current production entrypoints不得import。
- Account isolation：为避免Stage4 facts进入现有Stage3 Account后被v2 writer丢失，S4-08.2使用明确dormant full Account（inventory/coins/completed IDs/claims/Benben），复用现有domain values；Stage3 Account仍是唯一production shape。S4-08.4必须原子收敛为full Account唯一production authority，旧shape删除或仅私有作legacy/v2 interpretation，不得永久双truth。
- Fresh/migration：no-save纯构造`INITIAL_ACCOUNT`（items `1/2/1/1`、coins0、空progression/claims/Benben）+ null Attempt，不造Save/revision/Run、不调RNG或写盘；no-save≠committed revision0。v1/v2严格保留旧inventory并使用migration defaults，Rewards=[]、card=null、缺provenance为null，不得注入fresh bonus、补Reward或retro settlement。
- Factory：给定validated level、exact runId与adopted provenance/seed，按同一seed依次生成Board、canonical Run/RunItems，再以final Board + same adopted seed + validated Reward config调用唯一Reward generator，最后card=null、terminal=not-applicable并aggregate validate。Factory不做access/Start/Retry/Restart、ID/seed selection、4096 search、Account mutation或persistence。
- Validation/mapping：Value层复用现有inventory/Stable ID/Benben/Run/Board/RunItems/Reward/card/terminal/provenance；aggregate层只检查Reward↔Board/claim、one-time synchronization、phase↔terminal、card↔terminal/same-level used等跨值规则。Runtime≠DTO；mapper显式接收revision、逐字段no-alias映射并原样保留IDs；reconstruction显式建domain values，不查current catalog、不调RNG/Board或Reward generation。历史unknown level、不同Board/config、Rewards=[]与null provenance仍可恢复。
- Migration-only legacy trust：`legacy-excluded`唯一可信来源是strictly validated v1/v2经pure migration产生的in-process result；committed/head/backup/slot/revision均不提供semantic trust，也没有caller boolean或persisted trust flag。所有direct v3（包括committed head/head-backup）必须ordinary strict validate且legacy-excluded invalid。Runtime仍可表示trusted migrated won/failed legacy + card null，但mapper必须structured reject为non-writable；没有trusted direct-v3 validator、trusted legacy mapper或legacy Runtime→v3 roundtrip。
- Dormant reader：Stage2只选择committed payload；reader只验证/解释。no-save→fresh；v1/v2→strict validation→pure migration→trusted reconstruction；v3→ordinary strict validation→reconstruction。Reader不选择A/B、不写/回写、不增revision、不操作lease/slot/head、不调RNG/generation。未来legacy terminal仅能先dismiss清Attempt或Retry/合法replacement产生非legacy candidate再写v3；不得转settled、retro-settle或保留legacy写入。
