...
- 人工验收：`PASS — S4-08.3 MUTATION/LIFECYCLE CONTRACT READY`；包含初始 Design Review 及 waiting/phase、historical null provenance、Replay/Next 三项修正，不能只用旧版矩阵。Design closeout 仅改治理文档；production/tests/config changes = 0。S4-08.3 将首次组合 Stage 2 committed authority、dormant Stage 4 Runtime、lifecycle/gameplay/Item resources/Reward/Victory/terminal/Benben/temporary cards 与 persistence failure semantics；merge 后仍不接 production，S4-08.4 才能 cutover。
- Future mutation authority：`intent + expectedRevision + expectedRunId|null + required stable creation facts -> read committed Stage2 authority -> dormant Stage4 reader -> exact revision/runId gates -> pure domain composition -> full Stage4 GameState candidate -> final aggregate validation -> Runtime-to-Save-v3 mapper -> narrow dormant testable commit boundary -> only commit success returns publishable Runtime`。UI/caller snapshot 不是 authority。Candidate 不等于 committed state；load/corruption/revision/runId/mapper/lease/ownership/storage/commit 任一步失败保留旧 committed authority，failure 不带 publishable candidate。
- No-save/identity：no-save 不等于 committed revision 0；`expectedRevision=null`、account-only `expectedRunId=null`、首次成功 revision 0；corrupt/invalid 绝不当 no-save 或自动 fresh reset。Attempt command 的 expectedRunId 必须 exact-match committed Attempt，account-only 必须为 null；stale revision、stale runId、两者均 stale、null/Attempt 错配、replacement 后旧 runId 均先重读 committed authority 后拒绝。Stable ID 只做 exact equality，不 trim。
- Creation/config：runId、new Attempt base/adopted seed、首次成功 Detection 所需 seed 等 creation facts 在同一 intent 中稳定，或来自现有可注入 deterministic authority；commit 失败后重试同一 intent 不重抽，不让 `Math.random()`、`Date.now()` 或 uncontrolled UUID 进入 pure orchestration。Caller 只可提供 target levelId/必要稳定事实，不可提供任意 Board/Reward config；配置来自 validated production catalog。
- Phase/position authority：`active` 不等于单一位置；W=`waiting`，O=`on-board`，M=`revealed-mine-occupancy`，P=`pending-mine-encounter`。Movement、Run-level Flag、Abandon、Restart、Airplane 对 W/O/M 合法；Movement/Flag/Airplane 在 P 或 terminal 拒绝；Abandon/Restart 在 P 仍合法、terminal 拒绝。Detection 仅 O/M；Revive 仅 P；Pending-to-Failure 仅 P；全部 attempt command 对 account-only 拒绝。Benben Claim 仅 W 且 `hasTakenStep=false`、同关 available、无现卡及 pure Claim 其它条件。Retry 仅 settled failed / trusted legacy failed；terminal dismiss 仅 settled/legacy won/failed；Start 仅 account-only。Automatic Lucky 是内部 composition，不是 public command。
- Waiting：Start 创建真实 active+waiting Attempt。首步 Safe/Mine movement、Flag、Airplane、Abandon、Restart 均需可达；满足条件可 Claim，不要求先移动。Waiting Airplane 不是 actual movement：非终局时 position=waiting、`hasTakenStep=false`；可探索 Safe/揭 Mine/领 Reward/直接 won；若未终局且 Claim 条件仍满足，仍可 Claim。Waiting Detection 拒绝，服从现有 pure rule。
- Start/level switch：Start 从 committed read/fresh bootstrap、null revision/run gate、catalog lookup/unlock/config/creation facts、Complete Attempt Factory、full candidate、map、commit 形成新 Attempt；有 Attempt 时不得 silent replacement。Active/pending 切换关卡先可做 harmless target prevalidation，再分别 commit Abandon 与 Start；若 Abandon 已成功而 Start 失败，account-only 是合法 authority。Abandon W/O/M/P 只清 currentAttempt，完整保留 Account，不算 Failure、不 settlement/refund/rollback Reward/restore Benben；temporary card 随 Attempt 消失。
- Replay/Next/dismiss：account-only 已完成关卡重玩通过普通 Start，UI 可称 Replay，但不建第二条 account-only Replay rule。Terminal Replay 为单次 atomic replacement，通常来自 settled won、目标为 same exact level 且 Account 有 exact completion；trusted legacy won 仅当 Account 独立已有该 completion 才可 Replay，legacy 本身不补 completion、Reward 或 entitlement。Next 同样 atomic replacement，仅 won terminal 且 immediate next catalog level 已由 committed Account + catalog order 解锁；trusted legacy won 只有 Account 独立允许才可 Next，不 retro-complete。当前 catalog 仅 level-001，故现在 Next unavailable；未来追加 level 按 catalog 推导，不硬编码永久末关。Replay/Next commit 失败旧 terminal 保留。Terminal dismiss 清 settled/legacy won/failed Attempt，不重新 settlement、计失败、领 Reward 或补 completion。
- Restart/Retry：Restart 仅 W/O/M/P；Retry 仅 settled failed/trusted legacy failed。均同关 new runId/new adopted seed/new Board/new Rewards/fresh RunItems/card=null，Account 保留；Retry 旧 Failure 不再 settlement，legacy 不 retro-settle；Restart 不计 Failure、不重置 Benben streak、不退款/回滚 claimed assets。与旧 Board 比较**实际 Mine coordinate set**，不得只比 seed。旧 provenance 非 null 时保持 Stage2 `(old.seed+1) mod 2^32` 起、最多 4096 次 deterministic bounded search 及 goldens；旧 provenance=null 是合法历史事实，不单独禁用 Restart/Retry，使用本次 intent 已验证的 stable new base seed 起最多 4096 次搜索，不伪造旧 seed 为 0。最终 adopted seed 同时驱动 Board 与 Reward，commit retry 不更换。历史 levelId 即使不在当前 catalog 仍可 load/refresh/dismiss；缺当前 config 时仅拒绝需创建新 Attempt 的 Restart/Retry/Replay/Next。
- Flag/movement/Reward：Flag 必须委托 `setRunFlagged`，changed 才写；illegal/unchanged zero-write，不发 Reward/terminal/资源且保留其它 Stage4 facts。Safe movement 委托 `moveCharacter`，比较新旧 Board，所有 newly explored Safe 先走 S4-06 Reward compatibility/Account assets/claims，再按已有 Victory authority 处理首次 won，同 candidate 写 completion、Benben win、`terminalDisposition=settled`、card=null；最后 aggregate validate/map/commit。已探索 Safe revisit 若合法可提交位置变化，但不重复 Reward。一次 mutation 多 Reward 必须原子处理；coin/item overflow、one-time inconsistency 或 Reward invalidity 全部拒绝，不部分发放；historical/direct v3 one-time Reward path 必须保留。Reward-before-terminal，最后 Safe Reward 不得漏。
- Mine/Lucky/Failure：Mine movement 先产生 canonical pending，可作内部 composition fact，再用 `encounter.occurredOnFirstStep` 判断 Lucky。首步且有匹配未消费 temporary Lucky 优先，否则 Account Lucky；适用时同 mutation pending->survival，不 commit 中间 pending。首步 Safe 不消费 temporary Lucky，之后不能触发，但卡可留到 Attempt 结束。Later-step Mine 即使有 Lucky 也不触发/消耗/阻止 Revive，提交 pending 而非 Failure。Failure 是独立 P-only action，不由 Revive拒绝/库存不足自动触发：pending->failed Run->S4-07R settlement probe->仅 roll boundary 根据 exact levelId/runId 派生 deterministic eligibility->settlement->Account Benben->settled terminal/card=null->validate/map/commit。unavailable streak `0->1->2->roll`；roll success available+0、failure unavailable+0；available/used 不再累计/roll。pending/Restart/Abandon/persistence failure 不计失败；同一 authority/intention retry 不 reroll；成功资格只产生 Account available，terminal 不造临时 card。
- Victory/Benben：首次 won 同 candidate 写所有新 Reward claims、exact completedLevelIds、Benben win、settled terminal、card=null；无独立 completion payout，旧关重复胜利不重复插入 completion，ID 顺序遵守既有 authority，不自行 trim/sort/normalize。Benben win：no record 保持 absent；unavailable streak->0；available/used 各保持；不 roll。
- Items：Detection 只 O/M，复用既有 target selection，优先 temp Detection，再 Account；成功揭一 Mine/必要时清 flag、usage+1、seed 恰推进一次，无 Reward/Victory/terminal；no target 不扣资源、不增 usage、不推进 seed/Account/card，也不 commit。Airplane W/O/M，temp 优先，usage 上限 1；合法无收益仍消耗并提交；resource/usage->Board transform->new Safe Rewards->Victory->completion/Benben win->settled/card=null->validate/map/commit。Revive P-only，temp 优先，再 Account；成功消耗一种来源、usage+1、复用 canonical survival、Mine reveal/occupancy，绝不触发 Failure/Reward/Victory；wrong phase/cap/no resource/stale/commit failure 不把 P 自动变 failed。四类 temporary card 仅 matching unconsumed 优先；mismatch/consumed 回退永久库存，temp success 只把 card.consumed=true，不双扣；普通 usage 照增，Lucky 无 usage counter。
- Benben Claim：W + `hasTakenStep=false` 且满足 pure Claim 条件，caller 不能指定 card type；同 candidate Account available->used、Attempt 生成 deterministic unconsumed card。失败保持 available+no card，同 Attempt 重试同一卡。Claim 与 first movement 基于同一 revision/runId 竞争，至多一个成功；另一个重读 authority 后 stale/timing reject，不建 Claim lock。Waiting Airplane 非终局后若依然符合条件，仍可 Claim。
- Legacy/writer：`legacy-excluded` 仅可信 v1/v2 migration 来源，不 retro-settle、不转 settled；mapper 拒写仍保留 legacy 的 candidate，不能 fallback v2、drop Attempt、auto dismiss。只能通过 terminal dismiss、failed Retry、具备 entitlement 的 Replay/Next 等 REMOVE/REPLACE 变为 non-legacy candidate 后写 v3。Refresh 不是 mutation：Stage2 selected authority->dormant Stage4 reader->reconstructed Runtime；no-save fresh bootstrap，零 map/write/revision increment/RNG/gameplay mutation。
...
- Implementation decomposition：一个受控 PR，因 dormant Account/Attempt/GameState/factory/mapper/reconstruction/reader 强耦合；整个 PR 必须保持 dormant，production behavior 0 change。历史入口 S4-08.2 Implementation 已由下方 implementation PASS/CLOSED 取代；当前唯一入口以上方“唯一下一行动”为准。

### Stage 4 / S4-08.2 — Runtime Aggregate, Fresh Bootstrap, Complete Attempt Factory & Save v3 Mapping/Reconstruction Foundation Implementation — PASS / CLOSED

- 人工验收与实现稳定点：`91e2ed0778cccd8c2244d24ebb0d20b3a51b5d5b`；PR #48 已合并，main baseline `021baa82cd9233a1cc21048c5fcd8da7d44441b1`。S4-08.2 仅建立 dormant foundation，不切换 production authority。
- Dormant Runtime：完整 Account 包含 inventory、coins、completedLevelIds、oneTimeClaimIds、benbenByLevel；`Stage4GameState = { account, currentAttempt: AttemptState | null }`，account-only 合法。Attempt 唯一拥有 runId、levelId、provenance、run、runItems、rewards、temporary card、terminalDisposition；aggregate validation 约束所有权与组合一致性。S4-08.4 必须将 dormant Account 收敛为唯一 production Account authority，不得保留双真值。
- Fresh/migration/factory：fresh no-save Account inventory 为 `1/2/1/1`、coins 0、其余事实空，Attempt 为 null；旧 v1/v2 migration 保留旧 inventory，不注入 fresh bonus。Complete Attempt Factory 使用已采纳的 seed 构造同一 Board/Reward Attempt，不在 restore 时重新生成。
- Runtime/Save：显式 Runtime↔Save v3 映射与验证重建，不共享可变引用；`legacy-excluded` 仅由可信旧版本迁移获得，ordinary direct v3 不可信，legacy Attempt 不可写。Dormant reader 保持只读；production 仍使用 Stage 3 Runtime / Save v2，`CURRENT_SAVE_VERSION=2`，v3 writer disabled，Stage 2 persistence authority chain 未改变。
- 门禁证据：Unit `883/883`、Integration `110/110`、Playwright `1/1`，Architecture/TypeScript/Build/完整 quality PASS；A–AC 共 29 项 mutation sanity 均杀死，临时 mutation 未提交。Independent Reviewer PASS；branch Linux `35512885462`、PR required `35512989615`、main Linux `35513096076` 均 Success。
- 剩余边界：S4-08.3 尚须设计并验证完整 dormant gameplay/economy/lifecycle mutation orchestration；S4-08.4 才允许原子切换 production Runtime/reader/entrypoints/writer/version，S4-09 为 compatibility gate。S4-08.3 当前仅获 Design Review 入口，未批准 implementation。
...
- 人工验收结论：`PASS — S4-08C CLAIM/ITEM COMPATIBILITY CONTRACT READY`。第一次 Design Review 中把 Lucky resource existence 与 gameplay applicability 混同的表述，已在同一 Design 阶段内完成技术纠正；最终 authority 以本节修正合同为准。这不是 Product Contract change，而是恢复已冻结的“Lucky only applies to first actual step Mine”。Design closeout production/tests/config changes = 0。
- Claim eligibility：仅 current same-level Attempt、exact matching Account Benben `available`、card=null、hasTakenStep=false、真实 Run phase=active 时允许。missing/unavailable/used、wrong level、existing card、已 step、pending/failed/won 或 invalid authority 均明确拒绝；拒绝不消费 entitlement、不 derive/create card、不改 inventory/runItems/Board/Run，也不返回 publishable candidate。entitlement 保持 available，可留给未来 same-level Attempt。
- Atomic Claim：成功结果必须同时表达 matching Benben `available -> used` 与 Attempt card `null -> { item: deterministic S4-08A result, consumed:false }`；不得把 `used+null` 或 `available+card` 暴露为本次成功中间态。`used+null` 仍是合法 durable state，但不能补领。
- Card derivation/trust：Claim eligibility 先验证，真正成功时才基于 exact authoritative levelId/runId 调用现有 `deriveBenbenCard`；caller不得选择 item，不得使用 `Math.random`、第二 RNG、复制权重或 arbitrary config。同一 Attempt 重试必须得到同一卡。S4-08 最终负责证明 IDs 来自 committed currentAttempt。
- Resource model：matching unconsumed temporary card -> Account permanent inventory -> unavailable。允许一个极小 pure helper只负责 availability/source 与 successful-use consumption；temporary success 设 consumed=true且不扣 inventory，permanent success只扣对应 inventory且card不变，禁止双扣。helper不得决定 phase、target、encounter、first-step、usage cap、applicability或gameplay success；必须先确认 action属于 successful use，再原子消费资源。
- Item compatibility：Detection no-target不消费、不增usage、不推进seed，成功仍受总cap 2；Airplane合法no-effect仍成功消费且总cap 1；Revive仍pending-only/optional/cap 1，decline与拒绝不消费；Lucky仍automatic且无新usage counter。temporary资源只替代来源，不改变任何Stage 3 gameplay、Board/Run、target、Victory或usage规则；mismatch/consumed card不供资源，inventory可fallback，temporary不得提供额外quota。
- Correct Lucky applicability：pending创建后hasTakenStep已经为true；first-step canonical fact只能是 `encounter.occurredOnFirstStep`。Lucky applicable当且仅当当前为pending且该fact=true；此后才按 temporary Lucky -> Account Lucky 查资源。first Safe step 后 unconsumed temporary/permanent Lucky在later-step Mine不触发、不消费、也不阻止Revive。
- Correct Revive priority：仅 `occurredOnFirstStep=true` 且存在真正可用Lucky resource时，Lucky priority阻止Revive。later-step pending无论temporary/permanent Lucky是否存在，Revive都只按自身pending legality、usage cap与temporary Revive -> Account Revive资源判断。正常movement seam在first-step Lucky适用时应在pending发布前自动结算；Revive command仍防御非法/低层first-step pending调用。一个Attempt最多一张temporary card，测试不得构造temporary Lucky与temporary Revive并存。
- Atomic composition seam：successful Item future candidate必须一起表达gameplay effect、Board/Run、usage和选中资源消费。Airplane导致win时，未来S4-08必须按 effect/resource -> Reward -> terminal settlement -> card expiry -> aggregate validation -> Save/commit 组合，不能持久化terminal+card中间态。Lucky/Revive survival保持active并保留consumed=true card，直到Attempt结束。
- Save/Runtime boundary：S4-08C不修改Save v3 schema、migration、Reward、S4-07R terminal、`CURRENT_SAVE_VERSION`、writer或persistence path；不创建shadow GameState/Account/Attempt、temporary inventory、Manager/Provider/Bus/framework。S4-08B contract已足够表达最终Claim/use facts；若Implementation发现必须再改schema则BLOCKED。完整aggregate、terminal/new-attempt wiring、mapper与guarded persistence activation仍属S4-08。
- Test/mutation contract：除Claim/resource/四Item/usage/immutability/regression测试外，必须证明pending hasTakenStep=true但occurredOnFirstStep=true仍适用Lucky；first Safe后later Mine允许Revive且Lucky不耗；first-step temporary/permanent Lucky均优先Revive；无Lucky进入Revive；later-step Lucky不能救。必须杀死resource-existence-only Revive guard、pending hasTakenStep误判、later-step Lucky触发/扣减、first-step绕过Lucky以及auto flow先发布pending等mutations，恢复后不得提交。
- Preserved boundaries：S4-08A goldens、S4-07R、S4-08B Save contract、无temporary card时Stage 3行为、Reward、Stage 2 persistence与RNG isolation必须全部回归；version仍2、writer仍disabled。
- Revised order：`S4-08A CLOSED -> S4-07R CLOSED -> S4-08B CLOSED -> S4-08C Design CLOSED -> S4-08C Implementation -> S4-08 -> S4-09`。不得跳过Implementation gate或直接进入S4-08/S4-09。

### Stage 4 / S4-08B — Temporary Benben Card Attempt / Save v3 Contract & Persistence Foundation — PASS / CLOSED

- 人工验收结论：`PASS — S4-08B SAVE/ATTEMPT FOUNDATION IMPLEMENTED`。Implementation commit `c6c48bc08931437e053ed1341b87b24b6606c2a7`；PR #39；merged main baseline `bfbcba52b56b842ceecdea826fd2adf781e79f81`。S4-04 Save v3 继续 `PASS / CLOSED`；本次是 production v3 writer 首次启用前的正式 pre-production amendment，不是 S4-04 rollback、Save v4 或 production v3→v4 migration。
- Temporary-card authority：immutable Attempt-owned value 为 required nullable `null | { item, consumed }`；item 复用 Lucky/Detection/Airplane/Revive canonical authority。它不属于 Account/inventory、RunItems、Board 或 Benben progression，不保存 levelId/runId、seed/roll、claim/timestamp、UI 或 RNG state。
- Current Save v3 contract：`AttemptSaveV3.temporaryBenbenCard` 必须存在；缺失字段 invalid，无 missing→null fallback。非空 card 只允许 item 与 boolean consumed；validated value immutable/no-alias。Save v3 Benben canonical state 为 unavailable streak `0|1|2`、available/used streak `0`；load 不 clamp、repair、reset、roll 或 unlock。
- Aggregate/lifecycle invariants：card 存在必须由 exact same-level Account Benben `used` 授权；available、unavailable、missing 或 other-level used 均不能授权。反向不成立，`used + null` 合法。active/waiting 与 pending 可持 card；won、failed、legacy-excluded 必须 card=null。`hasTakenStep=true + unconsumed temporary Lucky` 是合法 durable state。
- Consumed separation：consumed=true 保留到 Attempt lifecycle 结束，用于区分“从未有卡”和“已消费”；不得自动置 null。card consumed 与 RunItems usage counters 是独立 authority facts，不能互相推断。
- Migration/restore：v1→v2→v3 与 v2→v3 明确生成 card=null；不 retro-roll/grant/draw、推断 consumed、调用 RNG 或写 storage。Refresh 只恢复 committed item+consumed，不 redraw/reroll/rederive；legacy-excluded 保持可信 migration-only boundary 且 card=null。
- Production boundary 未改变：`CURRENT_SAVE_VERSION=2`，production v3 writer `DISABLED`，production Runtime root 仍是 Stage 3 authority；无 Account/currentAttempt activation、Claim command、temporary-first Item resource、terminal/new-attempt wiring、新 persistence path、shadow GameState、Manager/framework 或第二 RNG。
- Quality evidence：Targeted `117/117`、extended compatibility `359/359`、Unit `773/773`、Integration `98/98`、Total `871/871` PASS；Architecture、TypeScript、Build、Playwright PASS。11 项 mutation sanity（missing fallback、unknown item、streak 3、wrong-level authorization、terminal card、migration grant、load RNG、aliasing、consumed reset、after-step Lucky 等）全部被杀死、恢复且未提交。
- Review/CI：Independent Reviewer PASS；branch Linux Quality `35204622298`、PR Linux Quality `35205015584`、main Linux Quality `35205168535` 均 Success。
- Remaining production gaps：Benben Claim；atomic available→used + deterministic card；matching temporary-card-first Item resource；四 Item compatibility；production Start/Restart/Retry card=null；terminal expiry wiring；full `GameState { account, currentAttempt }` activation；Runtime↔Save v3 mapper/reconstructor activation；production v3 writer switch。以上均未完成。
- S4-08C frozen product inputs：Claim 仅限 same-level available、matching current Attempt、card=null、hasTakenStep=false 且非 pending/failed/won；Claim 手动且 entitlement 可延后到未来 same-level Attempt。成功必须 atomic available→used + deterministic `{ item, consumed:false }`。matching unconsumed temporary card 优先于 Account inventory；usage caps 不变；mismatch/consumed 回退 inventory；inventory=0 但 matching temporary 可使用。Claim 与 Item action 分离；S4-08C 仍不得启用 writer/version/root。
- Revised order：`S4-08A CLOSED -> S4-07R CLOSED -> S4-08B CLOSED -> S4-08C Design/Implementation -> S4-08 -> S4-09`。不得重新执行已关闭任务、跳过 Design Review 或合并为巨型 PR。
...
- Trust/composition：TypeScript value 不证明完整 authority；S4-07R 只负责 state-machine legality。未来 S4-08 负责 `committed currentAttempt -> exact authoritative levelId/runId -> deriveBenbenEligibility -> settleTerminalOutcome`。推荐先无 eligibility 调用 settlement；仅收到 `eligibility-required` 后 derive 并二次纯调用，不新增 inspection/token/capability framework；第一次调用无 mutation、candidate 或 persistence side effect。
- Preserved S4-07 contract：只有最终 settled Failure 计数；won completion exact append-once；completion 将 unavailable 0/1/2 归零且保持 available/used；Restart/abandon 与 Lucky/Revive survival 不计数；settled/legacy idempotency、exact Stable IDs、immutability/no-aliasing 与 Reward-before-terminal 顺序保持。Won 不执行 eligibility RNG。
...
- 人工验收结论：`PASS — S4-08A BENBEN CONFIG & RNG FOUNDATION IMPLEMENTED`。Implementation / stable baseline `b715941869e6c1cbf206594f3a6eda698ebf0790`；PR #33。
- 唯一 global configuration authority 已实现为 immutable validated facts：`failuresPerRoll=3`、eligibility `3/10`、Lucky/Detection/Airplane/Revive weights=`3/3/1/3`、total weight `10`。无 per-level override、caller override、UI/Save duplicate 或 ConfigManager。
- Deterministic domains 与稳定输入：`benben-eligibility-v1`、`benben-card-v1`；输入为 exact `levelId` + exact current `runId`，不得 trim/normalize，按 UTF-8 与 byte-length-prefixed fields 编码，经 FNV-1a uint32 派生 seed，再使用独立 Mulberry32 与 rejection-sampling `nextInt`。不消费或修改 Mine、Detection、Reward RNG。
- 固定映射：eligibility roll `0..2` 成功、`3..9` 失败；card roll `0..2` Lucky、`3..5` Detection、`6` Airplane、`7..9` Revive。相同 stable inputs 必须 replay 相同结果；changed run/domain 必须隔离。
- Compatibility contract：domains、stable-ID composition/encoding、FNV-1a、Mulberry32、rejection sampling、bucket mapping 与 weights 均为兼容敏感合同。未来修改必须显式 change control、兼容影响与回归计划；golden failure 不得通过直接更新 expected 掩盖。
- 无新增持久事实：configuration、derived seed、roll、selection 或 RNG state 均不进入 Save；S4-08A 不实现 probabilistic terminal transition、temporary card Runtime/Save、claim、temporary resource priority、Runtime activation 或 v3 writer。
- 质量证据：Targeted `240/240`、Unit `733/733`、Integration `98/98`、Total `831/831` PASS；Architecture、TypeScript、Build、Playwright PASS。七项 mutation sanity（扩大 eligibility bucket、扩大 Airplane bucket、domain 混用、移除 runId、trim stable ID、modulo 替代 rejection、使用 `Math.random()`）均使目标测试失败，恢复后全绿且 mutation 未提交。
- Review/CI：Independent Reviewer PASS；branch Linux Quality run `35075041080`、PR run `35075292921`、main run `35075458657` 均 Success。Reverse Scan PASS：无第二套 config truth、RNG framework、Save schema/persistence path、terminal/card gameplay 或 writer activation。
- 修订依赖顺序：`S4-08A CLOSED -> S4-07R Design Review -> S4-08B -> S4-08C -> S4-08 -> S4-09`。在 S4-07R 中，历史 S4-07 仍保持 PASS/CLOSED，但旧 deterministic threshold branch 已被概率合同取代；只有 `streak=2 + real settled Failure` 需要消费可信 eligibility result，success 为 `available + 0`、failure 为 `unavailable + 0`，`available`/`used` 不 roll；completion、legacy exclusion、idempotency 与 Reward-before-terminal 顺序保持。S4-07R 不实现 temporary card/Save，production `CURRENT_SAVE_VERSION=2` 且 v3 writer 继续 disabled。
...
- 人工验收结论：`PASS — S4-07 TERMINAL COMPATIBILITY IMPLEMENTED, THRESHOLD AUTHORITY STILL PENDING`。Implementation commit `4bcb2808fce3f5ff68fc83d4740ff1cfd25b93b9`；PR #30；merged/final implementation baseline `bcf9e159a7d1233dcd065c30a6c7cdf29d84bc7a`。
- 已实现范围仅为 pure terminal compatibility：won/failed settlement、`completedLevelIds` 更新、Benben failure streak/entitlement transition、positive-safe-integer threshold validation/consumption、settled idempotency、`legacy-excluded` protection，以及 Reward-before-terminal composition evidence。没有 production persistence wiring、expanded production Account/future GameState authority、Save v3 writer、authoritative threshold source 或 Benben use gameplay。
- Won：`not-applicable + won -> settled`；completion exact-ID append once，Replay 不重复；本关 `unavailable` streak reset 为 0，`available`/`used` 保持，无 Benben record 仍 absent；不产生独立 completion payout。
- Failed：只有最终 settled Failure 计数；仅 `unavailable` 累积，达到 threshold 转为 canonical `available + 0`，`available`/`used` 不继续累计。Restart/abandon 不 increment/reset；pending、Lucky/Revive survival 不计 Failure。
- Legacy/idempotency：already settled 不得再次 settlement；`legacy-excluded` 不追溯 completion/streak/Reward。Legacy failed 可 Retry 但不补 streak；legacy won 不推断 completion。
- 顺序合同：`Run transition -> Reward compatibility -> terminal settlement -> complete candidate`。S4-07 helper 不读取 Reward payload，不拥有 Account/GameState/persistence authority。
- 真实证据：targeted terminal tests 44/44；Unit 690/690；Integration 98/98；Total 788/788；Architecture、TypeScript、Build、Playwright 均 PASS。六项关键 mutation 均使目标测试失败，恢复后通过且未提交。Independent Reviewer PASS；branch Linux `34823108754` Success；main Linux `34823643451` Success。
- Production boundary 保持：`CURRENT_SAVE_VERSION = 2`；production v3 writer `DISABLED`；production GameState root 未切换；production Account 未扩展 Stage 4 facts；terminal compatibility 未接 production persistence wrappers。
...
- Design closeout 当时的 Recovery 入口是 S4-07 Implementation；该入口现已完成并由上方 S4-07 PASS/CLOSED 记录取代。当前恢复必须以“唯一下一行动”和最新 closeout 为准，不得重新执行 S4-07。
...
- S4-06 closeout 当时未实现 production Reward gameplay/persistence wiring、terminal settlement、Reward generator、Level/Shop/Benben、Save v3 production write 与 Stage 4 economy activation；其中 pure terminal compatibility 已由后续 S4-07 PASS/CLOSED 完成，其余 production activation 边界仍未改变。movement/Airplane production wrappers 尚未调用 Reward compatibility；S4-06/S4-07 都是 S4-08 writer activation 的前置能力，不是 activation。
...
- S4-06 closeout 当时的下一硬前置是 S4-07；S4-07 Design/Implementation 现已 PASS/CLOSED。当前依赖顺序和唯一入口以上方最新 S4-07 closeout 为准：`S4-08A -> S4-08 -> S4-09`，不得重新执行 S4-07。
- S4-06 closeout 当时冻结的 S4-07 设计要求（Reward 先于 terminal、won/failed settlement、idempotency、overflow/invalid config、legacy exclusion、Retry 不重复累计）现已由 S4-07 实现并保留为合同，不再是待办入口。
- Historical Recovery superseded：S4-06 当时记录的“terminal 未实现 / 唯一 S4-07 Design Review”已失效。当前 Recovery 必须恢复 S4-07 PASS/CLOSED、production persistence 尚未接线、version 2/v3 writer disabled、threshold authority 缺失，以及唯一 S4-08A Design Review。

### Stage 4 / S4-06 — Reward Compatibility Design Review — PASS / CLOSED

- 人工验收结论：`PASS — REWARD COMPATIBILITY CONTRACT READY`。这是 DESIGN CONTRACT CLOSED；Reward claim gameplay、Reward generator、terminal settlement 与 production persistence 接线均未实现。
- Newly explored 只由 `old Board + next Board` 纯计算：old unexplored Safe -> next explored Safe；使用 deterministic row-major，不改 Stage 1 movement/Airplane API，不保存 exploration delta。只匹配该 delta 中 `claimed === false` 的 Reward；旧 explored/已 claimed 不重复领取；identity 仍为 `runId + coordinate`，无 rewardId。
- 同一 exploration transition 的多 Reward 必须先完整验证并完整计算，再一次性返回全部 assets 与 claimed facts；任一 invalid authority、one-time relation、safe-integer overflow 或 Account asset validation failure均整体拒绝。禁止 per-reward commit、partial payout、partial claimed state、clamp 或静默修复。
- Shadow-Account prohibition：S4-06 primitive 只接触短命 `RewardClaimAssets`（或等价值）中的 `inventory / coins / oneTimeClaimIds`，以及 `rewards / old Board / next Board`；成功返回 `nextInventory / nextCoins / nextOneTimeClaimIds / nextRewards / claimedCoordinates`。它不是 Account，不拥有 identity、revision、persistence、completed IDs、Benben 或 lifecycle；禁止 `FutureAccountState`、`RewardAccountState`、generic Account authority/constructor、generic patch/deep merge。S4-08 再由唯一 Account authority 的明确 adapter 应用结果。
- Asset contract：Coins 与四类 Item grant 均检查加法后的非负 safe integer；任一 overflow 整笔拒绝。`claimed=true` 与资产变化同一纯结果。non-null `oneTimeClaimId` 成功时必须同时加入 claim IDs；未 claimed 但 ID 已存在、claimed 但 ID 缺失、Attempt 内重复 non-null ID 均为 invalid authority，gameplay 不负责修复。
- Determinism：newly-explored 与 claim processing 均 row-major；Reward 输出数组保持原顺序；新 claim IDs 按实际领取 row-major 追加；不调用 RNG，Airplane 自身遍历顺序不得改变资产结果。
- Composition：movement/Airplane 的 Run transition 可先产生 won，再按 old/new Board claim Reward，最后把完整中间结果交给 S4-07 terminal settlement。最后一个 Safe 的 Reward 不得因已经 won 而跳过；Airplane 自身 inventory/usage 与全部 Reward 变化最终必须进入一个 candidate。Detection/Lucky/Revive/Mine/pending/failure 不探索 Safe，不触发 Reward claim。
- Error/idempotency：至少区分 claimed、nothing-to-claim 与 rejected（invalid authority/board transition/asset overflow）；nothing-to-claim 是正常结果，rejected 不携带部分结果。旧 explored 无 delta、claimed=true 无 payout；stale revision/runId 最终由 S4-08 authority-source gate 基于 committed Attempt 拒绝，不能信任客户端 claim truth。
- Persistence atomicity：最终链为 committed authority -> exploration -> Reward claim -> S4-07 terminal settlement -> complete GameState candidate -> Save v3 mapper -> guarded commit -> publish。Reward/terminal/revision/storage/lease/ownership 任一失败均不得发布 Safe explored、claimed、资产、claim ID 或 Item 消耗的部分结果；不设计 compensation transaction。
- Farming boundary：同 Attempt 同 Reward 仅一次；新 Attempt/Replay 可有新 Reward，失败后已合法获得资产保留。不新增 cooldown、daily cap、diminishing return 或 replay penalty；这些仍是 balance/farming 风险。
- Implementation test contract：movement 单 Reward/no Reward/claimed idempotency、Coins 与四 Item grant及 overflow、one-time 同步与非法关系、Airplane multi/mixed、整体 rejection、last-Safe-won handoff、非探索路径不 claim、row-major、immutability/no-aliasing 与 Stage 1–3 全回归。Mutation sanity 至少攻击 claimed gate、one-time update、overflow、Airplane all-reward 与 rejection atomicity，临时 mutation 不提交。
- Recovery：新 AI 仅读正式 repository authority 与 Git history，必须恢复 S4-05 CLOSED、S4-06 Design PASS/CLOSED、Reward generator 未实现、old/new Board delta、row-major/all-or-nothing、no shadow Account、primitive 仅操作 assets/rewards、movement/Airplane persistence 未接线、terminal 留 S4-07、v3 writer disabled/version 2，以及唯一 S4-06 Implementation。存在 GAP 时只修 docs。
...
- 人工验收结论：`PASS / CLOSED — RUNTIME FOUNDATION CONTRACT READY`。Design Review 为只读工作；production/tests/config/runtime/schema changes = 0，未启用 v3 writer，`CURRENT_SAVE_VERSION` 仍为 2。
- Future Runtime authority 冻结为 `GameState { account, currentAttempt: AttemptState | null }`。Account 始终存在；`currentAttempt = null` 是真实 account-only；非 null 是唯一 current attempt。禁止 fake Run、多 resumable attempts、run/runItems 独立 nullable 或第二套完整 Runtime authority。
- Attempt 唯一拥有 `runId / levelId / generationProvenance / run / runItems / rewards / terminalDisposition`；revision 继续属于 persistence context。Future Account facts 为 `inventory / coins / completedLevelIds / oneTimeClaimIds / benbenByLevel`。
- Item inventory 更新必须使用类似 `replaceInventory(account, inventory)` 的最小纯更新，完整保留其他 Account facts；不得要求每个 Item 重建或理解 Account 全部字段，不建立 generic patch/Manager/framework。
- `legacy-excluded` 只能从严格旧版本验证与可信迁移内部路径进入 Runtime；普通外部 v3 与普通 gameplay 不得声明或伪造。不得增加 persisted trust flag 或放宽普通 v3 validator。
...
- S4-04 当时的 Recovery 证据：正式资料可恢复 Stage 0–3 FROZEN、Stage 4 Product Contract FROZEN、S4-02 CLOSED、S4-03 CONTRACT FROZEN、S4-04 PASS/CLOSED、DTO-only scope、writer disabled/version 2/旧 Runtime、account-only validation、只读 migration/legacy、ID 原样保留、未实现 Reward/Benben gameplay、Stage 3 不变及 legacy trusted-source 后续保护。当前唯一入口以上方“唯一下一行动”为准。任何 GAP 仅修 authority docs，不开始 implementation。
