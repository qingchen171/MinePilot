...
### 1.1 工作目录写入门禁（2026-09-04 用户批准修订）

任何可能写入磁盘的项目命令执行前，必须先确认解析后的当前工作目录属于正式项目根目录 `D:\eliogames`（等于该目录或位于其内部）。若当前目录不符合预期，必须停止执行，修正工作目录并重新核验；不得依赖命令稍后创建或切换目录，也不得让 npm、Git 或其他工具向上搜索并误用项目根目录之外的配置。

在临时恢复副本中执行验证命令属于例外，但必须先确认该恢复目录已经存在、来源 commit/tree 已核对，并将命令的工作目录显式设置为该恢复目录。防错实现应保持简单，以命令执行前的路径解析与边界检查为主，不为此引入常驻服务或复杂系统。
